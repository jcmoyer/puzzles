...
procedure Day03 is
   type Token_Type is (Mul, Comma, LParen, RParen, Number, T_Do, T_Dont, Unrecognized);
   ...
   function To_Int (Source : String; Tok : Token) return Integer is
     (Integer'Value (Source (Tok.Source_First .. Tok.Source_Last))) with
     Pre => Tok.Kind = Number;
   ...
   function Lex (Source : String) return Token_Vectors.Vector is
      ...
      function Match_String (S : String) return Boolean is
        (Cursor + S'Length - 1 in Source'Range
         and then Source (Cursor .. Cursor + S'Length - 1) = S);

      procedure Lex_Lower is
      begin
         if Match_String ("mul") then
            Result.Append (Token'(Source_First => Cursor, Source_Last => Cursor + 2, Kind => Mul));
            Move (3);
            ...
            --  as `do` + `n` + `'` + `t`
            Result.Append
              (Token'(Source_First => Cursor, Source_Last => Cursor + 4, Kind => T_Dont));
            Move (5);
         elsif Match_String ("do") then
            Result.Append
              (Token'(Source_First => Cursor, Source_Last => Cursor + 1, Kind => T_Do));
            Move (2);
         else
            Result.Append
              (Token'(Source_First => Cursor, Source_Last => Cursor, Kind => Unrecognized));
            Next;
         end if;
      end Lex_Lower;
      ...
      begin
         ...
         Result.Append
           (Token'(Source_First => Number_First, Source_Last => Number_Last, Kind => Number));
      end Lex_Number;
   begin
      while Cursor <= Source'Last loop
         if Is_Lower (Source (Cursor)) then
            ...
            Result.Append (Token'(Source_First => Cursor, Source_Last => Cursor, Kind => LParen));
            Next;
         elsif Source (Cursor) = ')' then
            Result.Append (Token'(Source_First => Cursor, Source_Last => Cursor, Kind => RParen));
            Next;
         elsif Source (Cursor) = ',' then
            Result.Append (Token'(Source_First => Cursor, Source_Last => Cursor, Kind => Comma));
            Next;
         else
            Result.Append
              (Token'(Source_First => Cursor, Source_Last => Cursor, Kind => Unrecognized));
   ...
   function Eval_Muls
     (Source : String; Tokens : Token_Vectors.Vector; Handle_Do_Dont : Boolean) return Integer
   is
      ...
      begin
         --  bounds check
         if not (Cursor + A'Length - 1 in Tokens.First_Index .. Tokens.Last_Index) then
            return False;
         end if;
      ...
      Seq_Mul  : constant Token_Type_Array := (Mul, LParen, Number, Comma, Number, RParen);
      Seq_Do   : constant Token_Type_Array := (T_Do, LParen, RParen);
      Seq_Dont : constant Token_Type_Array := (T_Dont, LParen, RParen);

      function Match_Mul return Boolean is (Match_Sequence (Seq_Mul));

      function Match_Do return Boolean is (Match_Sequence (Seq_Do));

      function Match_Dont return Boolean is (Match_Sequence (Seq_Dont));

   begin
      while Cursor <= Tokens.Last_Index loop
         if Enable and then Match_Mul then
            Result :=
              Result + To_Int (Source, Tokens (Cursor + 2)) * To_Int (Source, Tokens (Cursor + 4));
